...
namespace Common.Structure.MathLibrary.RootFinding
{
    ...
    /// </summary>
    public static class VWDBMethod
    {
           ...
           double tolerance = 1e-8)
        {
            ...
            if ((fa > 0.0 && fb > 0.0) ||
                (fa < 0.0 && fb < 0.0))
            {
                return Result.ErrorResult<double>($"Root must be bracketed for {nameof(VWDBMethod)} to work.");
            }
            ...
            for (int iter = 0; iter < maxIterations; iter++)
            {
                ...
                {
                    b += RootFindingHelpers.SIGN(tolerance, xm);
                }
