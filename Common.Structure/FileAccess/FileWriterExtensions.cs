            ...
            switch (exportType)
            {
                case (ExportType.Csv):
                {
                    ...
                    foreach (T value in rowValues)
                    {
                        ...
                        WriteTableRow(writer, exportType, row.Where(info => headerValues.Contains(info.Name)).Select(ro => ro.GetValue(value)?.ToString()), headerFirstColumn);
                    }
                    break;
                }
                case (ExportType.Html):
