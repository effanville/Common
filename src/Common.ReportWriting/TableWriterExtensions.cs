...
namespace Effanville.Common.ReportWriting
{
    ...
    public static class TableWriterExtensions
    {
        ...
        {
            T? instanceForHeaderValues = default;
            foreach (var value in values)
            {
                if (value != null)
                {
                    var innerVal = value.FirstOrDefault();
                    if (innerVal != null)
                    {
                        instanceForHeaderValues = innerVal;
                        break;
                    }
                }
            }
        ...
        public static void WriteTable(
            ...
            bool headerFirstColumn)
        {
            if ((headerValues == null || headerValues.Count == 0) && (rowValues == null || rowValues.Count == 0))
            {
                return;
            }
