...
[TestFixture]
internal class ReportSplitterTests
{
    private static string enl = TestConstants.EnvNewLine;

    public static IEnumerable<TestCaseData> Tests()
    {
        yield return new TestCaseData(DocumentType.Md, "", new Document(DocumentType.Md, "")).SetName("Empty Md String");
        yield return new TestCaseData(DocumentType.Md, null, null).SetName("Null Md String");
        yield return new TestCaseData(DocumentType.Html, "", new Document(DocumentType.Html, "")).SetName("Empty Html String");
