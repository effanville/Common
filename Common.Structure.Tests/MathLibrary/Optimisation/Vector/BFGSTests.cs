...
namespace Common.Structure.Tests.MathLibrary.Optimisation.Vector
{
    ...
    public sealed class BFGSTests
    {
        ...
        public void CalcMin_Rosenbrock(double[] startingPoint)
        {
            ...
            Assert.That(Math.Abs(min.MinimisingPoint[0] - function.GlobalMinimum.Point[0]), Is.LessThan(1e-4));
            Assert.That(Math.Abs(min.MinimisingPoint[1] - function.GlobalMinimum.Point[1]), Is.LessThan(1e-4));
            Assert.That(Math.Abs(min.MinimisingValue - function.GlobalMinimum.Value), Is.LessThan(1e-4));
        }
        ...
        public void CalcMin_Himmelblau(double[] startingPoint, int localMinIndex)
        {
            ...
            Assert.That(Math.Abs(min.MinimisingPoint[0] - function.LocalMinima[localMinIndex].Point[0]), Is.LessThan(1e-4));
            Assert.That(Math.Abs(min.MinimisingPoint[1] - function.LocalMinima[localMinIndex].Point[1]), Is.LessThan(1e-4));
            Assert.That(Math.Abs(min.MinimisingValue - function.LocalMinima[localMinIndex].Value), Is.LessThan(1e-4));
        }
    }
}
